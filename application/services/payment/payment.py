...
class Card(BaseCardDetails):
    ...
    def verify_input(self, **kwargs):
        """
		IdNumber(mandatory, string, it should be a valid credit card number)
		...
		:return:
		"""
        cards_value = ["IdNumber", "CardHolder", "ExpirationDate", "Amount"]
        ...
        if (
            not type(
                kwargs["IdNumber"] == str
                and validate_card(kwargs["IdNumber"])
            )
            or not len(kwargs["IdNumber"]) == 16
        ):
    ...
    def __map_to_card(self, **kwargs):
        self.IdNumber = kwargs.get("IdNumber", None)
